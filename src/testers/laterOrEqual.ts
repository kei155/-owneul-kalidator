...
const laterOrEqualTester: Tester = {
  name: 'laterOrEqual',
  defaultMessageOnFailure: ':param(은/는) :$0(와/과) 같거나 늦은 날짜여야해요',
  handler: (value, extras, nullable, { fail }) => {
    ...
    if (extras.length !== 1) {
      return fail('laterOrEqual:compareDate 형식의 규칙을 정의해주세요')
    }
    ...
    return valueDate.isSame(compareDate) || valueDate.isAfter(compareDate)
  }
}

export default laterOrEqualTester
